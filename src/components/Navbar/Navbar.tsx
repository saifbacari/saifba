...
const Navbar: React.FC<NavbarProps> = ({ aboutRef, contactRef, workRef })  => {
  ...
  return (
    ...
    <label htmlFor="nav-toggle"   className={ `${styles.iconBurger} ${crossMenu ? `${styles.open}` : styles.iconBurger}`} 
    /*onClick={()=>{setCrossMenu(!crossMenu)}}*/>
    <div className={`${backdropVisible ? `${styles.line1}` : styles.line}`}></div>
    <div className={`${backdropVisible ? `${styles.line2}` : styles.line}`}></div>
    <div className={`${backdropVisible ? `${styles.line3}` : styles.line}`}></div>
    </label>
    </a>
